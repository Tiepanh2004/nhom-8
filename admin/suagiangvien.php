...
$sql_get = "SELECT * FROM nguoi_dung WHERE id = '$id' AND vaitro_id = 2";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['tendangnhap']) && !empty($_POST['hoten']) && !empty($_POST['namsinh']) && !empty($_POST['sdt']) && !empty($_POST['email'])) {
        ...
        $sql = "UPDATE `nguoi_dung` SET `ten_dang_nhap`='$tendangnhap', `ho_ten`='$hoten', `nam_sinh`='$namsinh', `sdt`='$sdt', `email`='$email' WHERE `id` = '$id' AND `vaitro_id` = 2";
        
        if (mysqli_query($conn, $sql)) {
            $successMessage = 'Cập nhật giảng viên thành công! Đang chuyển hướng...';
            $showForm = false;
            echo '<script>setTimeout(function(){ window.location.href = "index.php?page_layout=giangvien"; }, 1500);</script>';
        } else {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .form-header { background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); padding: 2rem; text-align: center; color: white; }
        ...
        .form-header h1::before { content: '\f044'; font-family: 'Font Awesome 6 Free'; font-weight: 900; }
        ...
        .box label i { margin-right: 0.5rem; color: #3b82f6; width: 20px; }
        ...
        .box input:focus { outline: none; border-color: #3b82f6; background: white; box-shadow: 0 0 0 3px rgba(59, 130, 246, 0.1); }
        .box input[type="submit"] { width: 100% !important; padding: 1rem !important; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%) !important; color: white !important; border: none !important; border-radius: 10px !important; font-size: 1rem !important; font-weight: 600 !important; cursor: pointer !important; transition: all 0.3s ease !important; box-shadow: 0 4px 12px rgba(59, 130, 246, 0.3) !important; margin-top: 1rem !important; }
        .box input[type="submit"]:hover, .box input[type="submit"]:focus, .box input[type="submit"]:active { color: white !important; background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%) !important; transform: translateY(-2px) !important; box-shadow: 0 6px 20px rgba(59, 130, 246, 0.4) !important; }
        ...
        .info-badge { display: inline-block; background: #dbeafe; color: #1e40af; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.85rem; font-weight: 600; margin-bottom: 1rem; }
    </style>
</head>
<body>
    <main>
        <div class="form-header">
            ...
            <p>Cập nhật thông tin giảng viên trong hệ thống</p>
        </div>
        <div class="form-body">
            <a href="index.php?page_layout=giangvien" class="back-btn">Quay lại danh sách</a>
            <div class="info-badge">ID: <?php echo htmlspecialchars($id); ?></div>
<?php if ($showForm && $teacher): ?>
            <form action="index.php?page_layout=suagiangvien&id=<?php echo $id; ?>" method="post">
                <div class="box">
                    <label for="tendangnhap"><i class="fas fa-user"></i>Tên đăng nhập</label>
                    <input id="tendangnhap" name="tendangnhap" type="text" value="<?php echo htmlspecialchars($teacher['ten_dang_nhap'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="hoten"><i class="fas fa-id-card"></i>Họ tên</label>
                    <input id="hoten" name="hoten" type="text" value="<?php echo htmlspecialchars($teacher['ho_ten'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="namsinh"><i class="fas fa-calendar"></i>Năm sinh</label>
                    <input id="namsinh" name="namsinh" type="date" value="<?php echo htmlspecialchars($teacher['nam_sinh'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="sdt"><i class="fas fa-phone"></i>Số điện thoại</label>
                    <input id="sdt" name="sdt" type="text" value="<?php echo htmlspecialchars($teacher['sdt'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="email"><i class="fas fa-envelope"></i>Email</label>
                    <input id="email" name="email" type="email" value="<?php echo htmlspecialchars($teacher['email'] ?? ''); ?>" required>
                </div>
                <div class="box">
...
<?php elseif (!$teacher): ?>
            <div class="error-message">Không tìm thấy giảng viên với ID này!</div>
<?php endif; ?>
<?php if ($errorMessage): ?>
