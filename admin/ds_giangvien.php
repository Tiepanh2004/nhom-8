...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        h1::before { content: '\f508'; font-family: 'Font Awesome 6 Free'; font-weight: 900; color: #10b981; }
        .nut { margin-bottom: 0; }
        .btn { display: inline-flex !important; align-items: center !important; gap: 0.5rem !important; padding: 0.75rem 1.5rem !important; font-size: 0.95rem !important; font-weight: 600 !important; color: white !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; border: none !important; border-radius: 10px !important; text-decoration: none !important; transition: all 0.3s ease !important; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3) !important; cursor: pointer !important; }
        .btn:hover, .btn:focus, .btn:active, .btn:visited { color: white !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; transform: translateY(-2px) !important; box-shadow: 0 6px 20px rgba(16, 185, 129, 0.4) !important; text-decoration: none !important; }
        ...
        th { background: linear-gradient(135deg, #10b981 0%, #059669 100%); color: white; padding: 1rem; text-align: left; font-weight: 600; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.5px; }
...
<body>
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
<?php
$sql = "SELECT `id`, `ho_ten`, `nam_sinh`, `sdt`, `email` FROM `nguoi_dung` WHERE `vaitro_id` = 2";
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo '<td>';
        echo '<a class="btn-edit" href="index.php?page_layout=suagiangvien&id=' . $row['id'] . '">Sửa</a>';
        echo '<a class="btn-delete" href="index.php?page_layout=xoagiangvien&id=' . $row['id'] . '" onclick="return confirm(\'Bạn có chắc muốn xóa giảng viên này?\')">Xóa</a>';
        echo '</td>';
        echo '</tr>';
    }
} else {
    echo '<tr><td colspan="6" class="empty-state"><i class="fas fa-inbox"></i><p>Chưa có giảng viên nào. Hãy thêm giảng viên đầu tiên!</p></td></tr>';
}
?>
            </tbody>
        </table>
