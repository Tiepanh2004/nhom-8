...
$sql_get = "SELECT * FROM nguoi_dung WHERE id = '$id' AND vaitro_id = 3";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['tendangnhap']) && !empty($_POST['hoten']) && !empty($_POST['namsinh']) && !empty($_POST['sdt']) && !empty($_POST['email'])) {
        ...
        $sql = "UPDATE `nguoi_dung` SET `ten_dang_nhap`='$tendangnhap', `ho_ten`='$hoten', `nam_sinh`='$namsinh', `sdt`='$sdt', `email`='$email' WHERE `id` = '$id' AND `vaitro_id` = 3";
        
        if (mysqli_query($conn, $sql)) {
            $successMessage = 'Cập nhật học viên thành công! Đang chuyển hướng...';
            $showForm = false;
...
<body>
    <main>
        <div class="form-header">
            ...
            <p>Cập nhật thông tin học viên trong hệ thống</p>
        </div>
        <div class="form-body">
...
<?php if ($showForm && $student): ?>
            <form action="index.php?page_layout=suahocvien&id=<?php echo $id; ?>" method="post">
                <div class="box">
                    <label for="tendangnhap"><i class="fas fa-user"></i>Tên đăng nhập</label>
                    <input id="tendangnhap" name="tendangnhap" type="text" value="<?php echo htmlspecialchars($student['ten_dang_nhap'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="hoten"><i class="fas fa-id-card"></i>Họ tên</label>
                    <input id="hoten" name="hoten" type="text" value="<?php echo htmlspecialchars($student['ho_ten'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="namsinh"><i class="fas fa-calendar"></i>Năm sinh</label>
                    <input id="namsinh" name="namsinh" type="date" value="<?php echo htmlspecialchars($student['nam_sinh'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="sdt"><i class="fas fa-phone"></i>Số điện thoại</label>
                    <input id="sdt" name="sdt" type="text" value="<?php echo htmlspecialchars($student['sdt'] ?? ''); ?>" required>
                </div>
                <div class="box">
                    <label for="email"><i class="fas fa-envelope"></i>Email</label>
                    <input id="email" name="email" type="email" value="<?php echo htmlspecialchars($student['email'] ?? ''); ?>" required>
                </div>
                <div class="box">
...
<?php elseif (!$student): ?>
            <div class="error-message">Không tìm thấy học viên với ID này!</div>
<?php endif; ?>
<?php if ($errorMessage): ?>
