...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['tendangnhap']) && !empty($_POST['hoten']) && !empty($_POST['namsinh']) && !empty($_POST['sdt']) && !empty($_POST['email'])) {
        ...
        $sql = "INSERT INTO `nguoi_dung`(`ten_dang_nhap`, `ho_ten`, `nam_sinh`, `sdt`, `email`, `vaitro_id`) VALUES ('$tendangnhap','$hoten','$namsinh','$sdt','$email', 2)";
        if (mysqli_query($conn, $sql)) {
            $successMessage = 'Thêm giảng viên thành công! Đang chuyển hướng...';
            $showForm = false;
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .form-header { background: linear-gradient(135deg, #10b981 0%, #059669 100%); padding: 2rem; text-align: center; color: white; }
        ...
        .box label i { margin-right: 0.5rem; color: #10b981; width: 20px; }
        ...
        .box input:focus { outline: none; border-color: #10b981; background: white; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1); }
        .box input[type="submit"] { width: 100% !important; padding: 1rem !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; color: white !important; border: none !important; border-radius: 10px !important; font-size: 1rem !important; font-weight: 600 !important; cursor: pointer !important; transition: all 0.3s ease !important; box-shadow: 0 4px 12px rgba(16, 185, 129, 0.3) !important; margin-top: 1rem !important; }
        .box input[type="submit"]:hover, .box input[type="submit"]:focus, .box input[type="submit"]:active { color: white !important; background: linear-gradient(135deg, #10b981 0%, #059669 100%) !important; transform: translateY(-2px) !important; box-shadow: 0 6px 20px rgba(16, 185, 129, 0.4) !important; }
...
<body>
    <main>
        <div class="form-header">
            ...
            <p>Điền thông tin chi tiết để thêm giảng viên vào hệ thống</p>
        </div>
        <div class="form-body">
...
<?php if ($showForm): ?>
            <form action="index.php?page_layout=themgiangvien" method="post">
                <div class="box">
                    ...
                    <input id="tendangnhap" name="tendangnhap" type="text" placeholder="VD: nguyenvana" required>
                </div>
                <div class="box">
                    <label for="hoten"><i class="fas fa-id-card"></i>Họ tên</label>
                    <input id="hoten" name="hoten" type="text" placeholder="VD: Nguyễn Văn A" required>
                </div>
                <div class="box">
                ...
                <div class="box">
                    ...
                    <input id="sdt" name="sdt" type="text" placeholder="VD: 0912345678" required>
                </div>
                <div class="box">
                    <label for="email"><i class="fas fa-envelope"></i>Email</label>
                    <input id="email" name="email" type="email" placeholder="VD: nguyenvana@example.com" required>
                </div>
                <div class="box">
