      ...
      <div class="container">
        <div class="login-contain" id="login-l">
          ...
          <div class="login-conten" id="login-form">
            ...
            $email = $_POST['email']; $pass = $_POST['password']; $sql = "SELECT
            * FROM `nguoi_dung` WHERE email='$email' AND mat_khau='$pass'";
            $result = mysqli_query($conn, $sql); $user =
            mysqli_fetch_assoc($result); if (mysqli_num_rows($result) > 0) {
            session_start(); $_SESSION["username"] = $user['ho_ten'];
            $_SESSION['userid'] = $user['id']; $_SESSION['vaitro'] =
            $user['vaitro_id']; if ($user['vaitro_id'] == 1) { header('location:
            admin/index.php'); } elseif ($user['vaitro'] == 2) {
            header('location: teacher/index.php'); } else { header('location:
            index.php'); } // echo "Hello " . $_SESSION['userid']; } else { echo
            "Ko ton tai!"; } } ?>
